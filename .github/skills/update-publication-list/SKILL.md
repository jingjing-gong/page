...
5. For every Scholar-listed paper that is missing, still marked as a preprint, or otherwise unclear, search the web to verify its latest status:
   ...
   - prefer primary sources such as the conference proceedings page, OpenReview, the publisher page, DBLP, ACL Anthology, arXiv comments, or the authors' project page
   - treat search results as evidence only when the title and author list align closely enough to rule out a different paper
...
12. When updating the Recent News section, keep entries in descending time order, with the newest verified item first.
13. When updating the Selected Publications section, include:
...
14. When writing author lines for the Selected Publications section, expand abbreviated names to full names whenever a reliable source is available:
...
- If Scholar only shows arXiv metadata, do not stop there; search the web before deciding it is still a preprint.
